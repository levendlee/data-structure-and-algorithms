    ...
    bool getPermutations(vector<int> &nums, vector<int> &cur, vector<bool> &visit)
    {
        ...
        for (int i = 0; i < 4; ++i)
        {
            if (!visit[i])
            {
                ...
                if (getPermutations(nums, cur, visit))
                    return true;
                cur.pop_back();
                visit[i] = false;
            }
...
public:
    bool judgePoint24(vector<int> &cards)
    {
        auto run_op = [](double a, double b)
        {
            vector<double> vec = {a + b, a * b, a - b, b - a};
            if (b != 0)
                vec.push_back(a / b);
            if (a != 0)
                vec.push_back(b / a);
        ...
        function<bool(vector<double> &)> backtracking;
        backtracking = [&](vector<double> &nums)
        {
            if (nums.size() == 1)
            {
                return abs(nums[0] - 24.0) < 1e-3;
            }
            ...
            for (int i = 0; i < n; ++i)
            {
                for (int j = i + 1; j < n; ++j)
                {
                    auto new_nums = run_op(nums[i], nums[j]);
                    vector<double> candidates;
                    for (int k = 0; k < n; ++k)
                    {
                        if (k == i || k == j)
                            continue;
                        candidates.push_back(nums[k]);
                    }
                    for (double nn : new_nums)
                    {
        ...
        for (int num : cards)
        {
            dcards.push_back(num);
        }
        return backtracking(dcards);
    }
};